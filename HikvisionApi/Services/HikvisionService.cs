...
namespace HikvisionApi.Services
{
    public class HikvisionService
    {
        ...
        // Notificador de eventos ANPR para el dashboard web.
        // Por defecto es un no-op (AnprEventBusNoop).
        // Si el dashboard está habilitado, se registra AnprEventBusSignalR en Program.cs.
        // Completamente opcional — HikvisionService no referencia SignalR directamente.
        private readonly IAnprEventBus _eventBus;
            ...
            IFormFile? plateImage, IFormFile? fullImage)
        {
            ...
            // ── PUSH AL DASHBOARD WEB ────────────────────────────────────
            // IAnprEventBus es no-op por defecto; si el dashboard está habilitado
            // se registra AnprEventBusSignalR que hace el push via SignalR.
            if (_eventBus is not AnprEventBusNoop)
            {
                string imagenUrl = "";
                try { imagenUrl = await imagenUrlTask; } catch { }

                _ = _eventBus.PublicarAsync(new
                {
                    placa,
            ...
            if (!_parqueadero.AbrirTodo && !tieneConvenio)
            {
                ...
                if (_parqueadero.EntregarTiquete && !string.IsNullOrEmpty(impresora))
                    _print.ImprimirTiqueteLocal(impresora, placa, tipoVehiculo,
                        horaEntrada, carrilNombre, false,
                        _cache.GetConfiguracion());
                return;
            }
